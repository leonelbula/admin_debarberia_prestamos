...
class ProductosVentas {
	...
	public function MostrarProductosId() {
		$sql = "SELECT * FROM productoventa WHERE id = {$this->getId()}";
		$resul = $this->db->query($sql);
		return $resul;
	}
	public function TotalProductos() {
		$sql = "SELECT COUNT(id) as total FROM productoventa ";
		$resul = $this->db->query($sql);
		return $resul;
	}
	public function MostrarProductos() {
		$sql = "SELECT * FROM productoventa ORDER by nombre DESC ";
		$resul = $this->db->query($sql);
		return $resul;
	}
	public function MostrarUltimoProductos() {
		$sql = "SELECT p.id , p.codigo FROM productoventa p ORDER by id DESC LIMIT 1";
		$resul = $this->db->query($sql);
		return $resul;
	}
	public function Guardar() {
		$sql = "INSERT INTO productoventa VALUES(NULL,'{$this->getCodigo()}','{$this->getNombre()}',{$this->getCosto()},"
...
//		}
	...
	public function Actulizar() {
		$sql = "UPDATE productoventa SET codigo='{$this->getCodigo()}',nombre='{$this->getNombre()}',costo={$this->getCosto()},"
	...
	public function Eliminar() {
		$sql = "DELETE FROM productoventa WHERE id = {$this->getId()}";
		$resp = $this->db->query($sql);
		$resul = FALSE;
		if($resp){
			$resul = TRUE;
	...
	public function VercantidadProducto() {
		$sql = "SELECT cantidad FROM productoventa WHERE id = {$this->getId()}";
		$resp = $this->db->query($sql);		
		return $resp;
	}
	public function ActulizarStock() {
		$sql = "UPDATE productoventa SET cantidad={$this->getCantidad()}  WHERE id = {$this->getId()}";
		$resp = $this->db->query($sql);
		$resul = FALSE;
		if($resp){
			$resul = TRUE;
	...
	public function ValorInventario() {
		$sql = "SELECT SUM(costo*cantidad) as resultado FROM productoventa";
		$resp = $this->db->query($sql);		
		return $resp;
	}
	public function stock() {		
		$sql = "SELECT COUNT(id) AS total  FROM productoventa WHERE cantidad_min >= cantidad";
		$resp = $this->db->query($sql);		
		return $resp;
	}
}