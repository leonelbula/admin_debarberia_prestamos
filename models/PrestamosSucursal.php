...
class PrestamosSucursal{
	...
	public function Guardar() {
		...
		. "'{$this->getFecha()}','{$this->getFecha_vencimiento()}','{$this->getDescripcion()}',{$this->getInteres()},"
		. "{$this->getValor()},{$this->getSaldo()},{$this->getPlazo()},{$this->getCuotas()},{$this->getUtilidad()},{$this->getValorcuota()})";
		$resp = $this->db->query($sql);
		$result = FALSE;
		if($resp){
			$result = TRUE;
	...
	public function Actulizar() {
		...
		. "fecha='{$this->getFecha()}',fecha_vencimiento='{$this->getFecha_vencimiento()}',descripcion={$this->getDescripcion()},"
		. "interes={$this->getInteres()},valor={$this->getValor()},saldo={$this->getSaldo()},plazo={$this->getPlazo()},"
		. "cuotas={$this->getCuotas()},utilidad={$this->getCuotas()},valorcuota={$this->getValorcuota()} WHERE id = {$this->getId()} AND id_sucursal = {$this->getId_sucursal()}";
		$resp = $this->db->query($sql);
		$result = FALSE;
		if($resp){
			$result = TRUE;
	...
	public function Eliminar() {
		$sql = "DELETE FROM prestamo_estilista WHERE id = {$this->getId()} AND id_sucursal = {$this->getId_sucursal()}";
		$resp = $this->db->query($sql);
		$result = FALSE;
		if($resp){
			$result = TRUE;
