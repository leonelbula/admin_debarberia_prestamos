...
public function ReportesVentasServicio($fechaInicial,$fechaFinal) {

		
if($fechaInicial == $fechaFinal){
$sql = "SELECT s.id,s.nombre, SUM(v.totalventa) AS ventaservicio ,SUM(v.utilidad) AS totalutilidad,sum(v.totalcosto) AS valorcosto FROM venta_producto v INNER JOIN sucursal s ON v.id_sucursal=s.id AND fecha LIKE '%$fechaFinal%' GROUP BY s.id";
...
$sql = "SELECT s.id,s.nombre, SUM(v.totalventa) AS ventaservicio ,SUM(v.utilidad) AS totalutilidad,sum(v.totalcosto) AS valorcosto FROM venta_producto v INNER JOIN sucursal s ON v.id_sucursal=s.id AND fecha  BETWEEN  '$fechaInicial' AND '$fechaFinalMasUno' GROUP BY  s.id";
} else {
$sql = "SELECT s.id,s.nombre, SUM(v.totalventa) AS ventaservicio ,SUM(v.utilidad) AS totalutilidad,sum(v.totalcosto) AS valorcosto FROM venta_producto v INNER JOIN sucursal s ON v.id_sucursal=s.id AND fecha  BETWEEN  '$fechaInicial' AND '$fechaFinal'GROUP BY  s.id";
}
}
...
$detalles = $reporte->ReportesVentasServicio($fechaInicial,$fechaFinal);
...
$bloque3 = <<<EOF
	...
	<table style="font-size:10px; padding:5px 10px; border: 1px solid #666;">
		...
		<th style="border: 1px solid #666; background-color:white; width:85px; text-align:center; font-weight: bold">Costo</th>
...
$nombre = $value['nombre'];
$venta = number_format($value['ventaservicio']);
$utilidad = number_format($value['totalutilidad']);
$costo = number_format($value['valorcosto']);
