...
$sql = "SELECT * FROM venta_servicio WHERE fecha LIKE '%$fechaFinal%'";
...
$sql = "SELECT * FROM venta_servicio WHERE fecha BETWEEN '$fechaInicial' AND '$fechaFinalMasUno'";
} else {
$sql = "SELECT * FROM venta_servicio WHERE fecha BETWEEN '$fechaInicial' AND '$fechaFinal'";
}
}
...
$totalUtilidad = $row->valorinterno;
...
$factura = $value['num_venta'];
$fecha = $value['fecha'];

$utilidad = number_format($value['valorinterno']);
$total = number_format($value['valor']);
