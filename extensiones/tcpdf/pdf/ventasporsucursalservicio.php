...
$sql = "SELECT s.id,s.nombre, SUM(v.valor) AS ventaservicio ,SUM(v.valorinterno) AS valorinterno FROM venta_servicio v INNER JOIN sucursal s ON v.id_sucursal=s.id AND fecha LIKE '%$fechaFinal%' GROUP BY s.id";
...
$sql = "SELECT s.id,s.nombre, SUM(v.valor) AS ventaservicio ,SUM(v.valorinterno) AS valorinterno FROM venta_servicio v INNER JOIN sucursal s ON v.id_sucursal=s.id AND fecha  BETWEEN  '$fechaInicial' AND '$fechaFinalMasUno' GROUP BY  s.id";
} else {
$sql = "SELECT s.id,s.nombre, SUM(v.valor) AS ventaservicio ,SUM(v.valorinterno) AS valorinterno FROM venta_servicio v INNER JOIN sucursal s ON v.id_sucursal=s.id AND fecha  BETWEEN  '$fechaInicial' AND '$fechaFinal'GROUP BY  s.id";
}
}
...
$utilidad = number_format($value['valorinterno']);
