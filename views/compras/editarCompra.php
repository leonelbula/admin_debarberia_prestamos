    ...
    <section class="invoice">
		<form role="form" method="post" action="<?= URL_BASE ?>compras/actulizarcompra" class="formularioCompra">
			...
			<div class="row invoice-info">
				...
				while ($row2 = $detallesCompra-> fetch_object()) {
					$idcompra = $row2->id;
					$numero_factura = $row2->num_factura;
					$tipo = $row2->tipo;
					$plazo = $row2->plazo;
					$idproveedor = $row2->id_proveedor;
					$fecha = $row2->fecha_compra;
					$productos = $row2->detalles;			
			...
			<div class="col-sm-4 invoice-col">
				...
				<div class="col-sm-4 invoice-col">
					...
					<div class="input-group">
						...
						<input type="date" class="form-control" value="<?= $fecha?>" name="fechafactura" required>
					</div>
					<div class="input-group">				
					</div>
					...
					<table class="table table-striped">
						...
						<tbody class="nuevoProducto">
							...
							foreach ($listaProducto as $key => $value) {
								...
								$productoDetl = productosController::VerProdutoId($idProdu);
								while ($row4 = $productoDetl ->fetch_object()) {									
									
									$stock = $row4->cantidad;
								}
								echo '<tr>
										<td class="valorivap">'.$value['codigo'].'<input  class="valoriva" type="hidden" name="valoriva"  value=""/></td>
				...
				<div class="col-xs-6">
					...
					<div class="col-xs-6">
						<div class="form-group">
							...
							<select class="chosen-select form-control seleccionarTipoCompra" name="tipoventa" id="form-field-select-3" required="">
								<option value="">Seleciones el Tipo</option>
								<option value="1"<?= $tipo == 1 ? 'selected':''?>>Credito</option>
								<option value="0"<?= $tipo == 0 ? 'selected':''?>>Contado</option>

							</select>
						</div>
						<div class="form-group">
							<label for="estado">Plazo en Dias</label>	
							<input type="number" class="form-control plazoVenta"  name="dias" value="<?= $plazo ?>" />							
						</div>
					</div>
				</div>
				<!-- /.col -->
